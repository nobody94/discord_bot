...
const { addMoney, getIcon,checkPay } = require("../utils/currency.js");
...
async function becaHandler(args, message, fishTank, tankKey, userId) {
  ...
  if (action === "sell") {
    ...
    if (!fishId)
      return message.reply(
        `${errorIcon} | Cú pháp: \`.beca sell [ID] [Số lượng]\` hoặc \`.beca sell all\``,
      );
    ...
    if (fishId === "all") {
      if (fishTank.length === 0)
        return message.reply(`${errorIcon} | Bể cá của bạn đang trống.`);
      ...
      if (soldCount === 0)
        return message.reply(
          `${errorIcon} | Không có cá thường nào để bán. Các loại cá hiếm đã được giữ lại an toàn!`,
        );
    ...
    // TRƯỜNG HỢP: BÁN LOẠI CÁ CỤ THỂ
      ...
      if (!fish)
        return message.reply(`${errorIcon} | Loại cá này không tồn tại.`);

      const countInTank = fishTank.filter((id) => id === fishId).length;
      if (countInTank === 0)
        return message.reply(
          `${errorIcon} | Bạn không có con **${fish.name}** nào.`,
        );

      if (args[2]?.toLowerCase() === "all") amountToSell = countInTank;
      else amountToSell = amountToSell || 1;

      if (amountToSell > countInTank)
        return message.reply(
          `${errorIcon} | Bạn chỉ có **${countInTank}** con.`,
        );
  ...
  if (action === "give") {
    ...
    if (!target || !fishId)
      return message.reply(
        `${errorIcon} | Cú pháp: \`.beca give @User [ID Cá] [Số lượng]\``,
      );
    if (target.id === userId)
      return message.reply(
        `${errorIcon} | Bạn không thể tự tặng cá cho chính mình!`,
      );
    ...
    const countInTank = fishTank.filter((id) => id === fishId).length;
    if (countInTank < amount)
      return message.reply(
        `${errorIcon} | Bạn không đủ **${amount}** con **${fish.name}** để tặng.`,
      );
