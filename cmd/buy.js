...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!itemId) {
            return message.reply('❌ | Vui lòng nhập ID vật phẩm muốn mua. Ví dụ: `.buy the_doi_ten`');
        }

        // 2. Kiểm tra vật phẩm có tồn tại trong SHOP_ITEMS không
        const item = SHOP_ITEMS[itemId];
        if (!item) {
            return message.reply(`❌ | Vật phẩm với ID \`${itemId}\` không tồn tại trong cửa hàng. Dùng \`.shop\` để xem danh sách.`);
        }
        ...
        try {
            ...
            if (success) {
                ...
                return message.reply('❌ | Đã xảy ra lỗi trong quá trình xử lý giao dịch.');
            }
        } catch (error) {
            console.error("LỖI KHI MUA ĐỒ:", error);
            return message.reply('❌ | Hệ thống gặp lỗi không xác định khi mua vật phẩm.');
        }
    }
};