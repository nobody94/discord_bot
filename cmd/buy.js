...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!itemId) {
            return message.reply(`${errorIcon} | Vui lòng nhập ID vật phẩm. Ví dụ: \`.buy gacha_box 5\``);
        }
        ...
        if (!item || item.hideFromShop === true) {
            return message.reply(`${errorIcon} | Vật phẩm này không tồn tại hoặc không bán trực tiếp!`);
        }
        ...
        try {
            ...
            if (success) {
                ...
                return message.reply(`${errorIcon} | Giao dịch thất bại do lỗi hệ thống.`);
            }
        } catch (error) {
            console.error("LỖI KHI MUA ĐỒ:", error);
            return message.reply(`${errorIcon} | Hệ thống gặp lỗi khi xử lý giao dịch.`);
        }
    }
};