...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (args[0] && ["sell", "give", "throw"].includes(args[0].toLowerCase())) {
            ...
            const trunkHandler = await baloHandler(args, message, authorBalo, authorBaloKey, authorId);
            if (trunkHandler) return;
        ...
        if (inventory.length === 0) {
            return message.reply({
                embeds: [new EmbedBuilder().setColor(0x3498db).setDescription(`🔑 Kho đồ của **${username}** đang trống rỗng...`)]
            });
        }
